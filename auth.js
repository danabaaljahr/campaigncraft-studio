(() => {
  ...
  const userKey = base => state.user?.id ? `${base}__${state.user.id}` : `${base}__guest`;
    ...
    return `
      <div class="auth-gate" id="authGate" aria-live="polite">
        <div class="auth-card">
          ...
          <p class="auth-note" data-i18n="auth_note">كل مستخدم يرى حملاته واستبياناته فقط. رابط الاستبيان العام يبقى متاحًا للمشاركين بدون تسجيل دخول.</p>
        </div>
      </div>`;
  }
  ...
  function accountControlsMarkup() {
    return `
      <div class="account-controls" id="accountControls">
        ...
        <button type="button" id="accountButton" class="account-button" title="Account"><span class="account-avatar">U</span><span class="account-name"></span></button>
  ...
  async function init() {
    ...
    if (!db) {
      showGate(state.language === 'en' ? 'Supabase is not configured.' : 'Supabase غير مهيأ.');
      return;
    }
